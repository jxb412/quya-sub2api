...
fn main() {
    ...
    #[cfg(target_os = "linux")]
    // The helper mutates process-wide OpenSSL environment variables before the
    // Tokio runtime starts; calling it here is intentionally single-threaded.
    unsafe {
...
async fn serve() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let controller = goplugin::Controller::new(shutdown_tx);
